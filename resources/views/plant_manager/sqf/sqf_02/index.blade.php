            ...
            <div class="row g-3">
                ...
                <div class="col-auto">
                    <div class="d-flex align-items-center gap-2 justify-content-lg-end">
                        ...
                        <a href="{{ route('plantmanager.sqf_2.add') }}" class="btn  px-4"
                            style="background-color: #034EA2; color: white;"><i class="bi bi-plus-lg me-2"></i>Add SQF 02</a>
                    </div>
                </div>
            </div>
        </form>
        <div class="card mt-4">
            <div class="card-body">
                <div class="product-table">
                            ...
                            <tbody>
                                ...
                                @foreach ($sqf02Record as $item)
                                    <tr class="text-center">
                                        ...
                                        <td width="100px">
                                            <a class="" href="{{ route('plantmanager.sqf_2.show', $item->id) }}">
                                                <i class="bi bi-eye-fill me-2"></i>
                                            </a>
                                            <a class="" href="{{ route('plantmanager.sqf_2.edit', $item->id) }}">
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-footer border-top-0">
                                                    <form action="{{ route('plantmanager.sqf_2.destroy', $item->id) }}" method="POST">
                                                        @csrf
                                                        @method('DELETE')
                    ...
                    @if ($sqf02Record->lastPage() > 1)
                        <nav aria-label="Page navigation example">
                            <ul class="pagination justify-content-end mt-3">
                                {{-- Previous Page Link --}}
                                <li class="page-item {{ $sqf02Record->onFirstPage() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $sqf02Record->previousPageUrl() ?? 'javascript:;' }}"
                                        tabindex="-1">Previous</a>
                                </li>

                                {{-- Pagination Links --}}
                                @for ($i = 1; $i <= $sqf02Record->lastPage(); $i++)
                                    <li class="page-item {{ $sqf02Record->currentPage() == $i ? 'active' : '' }}">
                                        <a class="page-link" href="{{ $sqf02Record->url($i) }}">{{ $i }}</a>
                                    </li>
                                @endfor

                                {{-- Next Page Link --}}
                                <li class="page-item {{ !$sqf02Record->hasMorePages() ? 'disabled' : '' }}">
                                    <a class="page-link" href="{{ $sqf02Record->nextPageUrl() ?? 'javascript:;' }}">Next</a>
                                </li>
                            </ul>
                        </nav>
                    @endif
